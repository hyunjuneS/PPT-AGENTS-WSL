...
def _get_lid_model():
    ...
    if env_path and Path(env_path).exists():
        _LID_MODEL = load_model(env_path)
        return _LID_MODEL

    # 2. Repo-bundled model (place lid.176.bin in offline_resources/fasttext/)
...
def language_id(text: str) -> Language:
    ...
    if model is None:
        ...
        lang, _ = langid.classify(text[:1024])
        return Language(lid=lang)
    return Language(
