...
def check_playwright_browsers():
    ...
    # Offline mode: skip download when PLAYWRIGHT_BROWSERS_PATH points to a bundled browser
    if os.environ.get("PLAYWRIGHT_OFFLINE") == "1":
        browsers_path = os.environ.get("PLAYWRIGHT_BROWSERS_PATH", "")
        console.print(
            f"[yellow]⚠[/yellow] Offline mode: skipping Playwright download "
...
def check_npm_dependencies():
    ...
    # Offline mode: install from pre-bundled tarballs instead of npm registry
    offline_nm = os.environ.get("OFFLINE_NODE_MODULES")
    if offline_nm:
        from pathlib import Path

        tarballs = list(Path(offline_nm).glob("*.tgz"))
        if tarballs:
            cache_dir = cache_nm.parent
            cache_dir.mkdir(parents=True, exist_ok=True)
            console.print(
                f"[yellow]⚠[/yellow] Installing Node.js deps from offline tarballs at {offline_nm}"
            )
            try:
                success = run_streaming_command(
                    ["npm", "install", "--prefix", str(cache_dir), *[str(t) for t in tarballs]],
                    failure_message="[yellow]⚠[/yellow] Failed to install from tarballs",
                )
                if success and all((cache_nm / pkg).exists() for pkg in required):
                    console.print("[green]✓[/green] Node.js dependencies installed from offline tarballs")
                    return True
            except FileNotFoundError:
